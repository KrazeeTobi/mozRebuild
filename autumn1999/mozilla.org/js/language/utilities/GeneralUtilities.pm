...
@ISA = qw(Exporter);
@EXPORT = qw($sep $pre $noSet arrayEq listToHash updateHash formatDate pathToAbsolute addDirectorySuffix pathDifference pathSum
			findFiles backUpFile updateFile);
...
sub addDirectorySuffix($) {
	local ($_) = @_;
	$_ .= $sep if $_ eq "" || substr($_, length($_)-1, 1) ne $sep;
	return $_;
}
...
sub splitPath($) {
	...
	if ($onMac) {
		die "Need absolute path" if substr($path, 0, 1) eq ':' || substr($path, length($path)-1, 1) eq ':' || /::/;
		return split(/:/, $path);
	} else {
		die "Need absolute path" if substr($path, 0, 1) ne '/' || substr($path, length($path)-1, 1) eq '/' || /\/\.\.?(?:\/|$)/;
		my @s = split(/\//, $path);
		shift @s;
		return @s;
	}
...
# Set the file's modification date $timeDelta seconds after its previous value.
sub updateFile($$@) {
	my $pathName = shift;
	my $timeDelta = shift;
	if ($noSet) {
